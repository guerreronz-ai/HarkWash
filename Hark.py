...
def get_status_info(service, reception_str, req_day_str, req_time_str):
    try:
        ...
        # ✅ Fallback automático para soportar datos antiguos (24h) y nuevos (12h AM/PM)
        try:
...
def page_ingress():
    ...
    with st.form( "ingress_form ", clear_on_submit=True):
        ...
        if st.form_submit_button( "💾 Save Vehicle ", use_container_width=True, type= "primary "):
            ...
            dallas_tz = ZoneInfo( "America/Chicago ")
            dallas_now = datetime.now(dallas_tz).strftime( "%Y-%m-%d %I:%M %p ") # ✅ Formato AM/PM
            ...
            with get_db() as conn:
                ...
                c.execute( """
                    ...
                    (vin_number, tag_number, brand, model, required_day, required_time, service, notes,
                 ...
                 """, (
                    ...
                    req_time.strftime( "%I:%M %p ") if req_time else None, # ✅ Formato AM/PM
                    service,
                    notes.strip(),
                    1 if urgent else 0,
    ...
    for svc, vehs in by_service.items():
        ...
        if st.button( " Done ", key=f "btn_deliver_{svc.replace(' ', '_')} ", use_container_width=True, type= "primary "):
            ...
            if selected_rows.empty:
                ...
                count = 0
                dallas_tz = ZoneInfo( "America/Chicago ")
                delivery_time = datetime.now(dallas_tz).strftime( "%Y-%m-%d %I:%M %p ") # ✅ Formato AM/PM
    ...
    with st.form( "guest_ingress_form ", clear_on_submit=True):
        ...
        if st.form_submit_button( "💾Save Vehicle ", use_container_width=True, type= "primary "):
            ...
            if req_type ==  "both " and (not vin.strip() or not tag.strip()):
                st.error( "❌ This service requires VIN y TAG ")
                st.stop()
            elif req_type ==  "vin " and not vin.strip():
                st.error( "❌ This service requires VIN ")
                st.stop()
            elif req_type ==  "tag " and not tag.strip():
                st.error( "❌ This service requires TAG ")
                st.stop()

            dallas_now = datetime.now(ZoneInfo( "America/Chicago ")).strftime( "%Y-%m-%d %I:%M %p ") # ✅ Formato AM/PM

            with get_db() as conn:
                c = conn.cursor()
                c.execute( """
                    INSERT INTO vehicles 
                    (vin_number, tag_number, brand, model, required_day, required_time, 
